...
class REST:
    ...
    def submit_order(self, symbol, qty, side, type, time_in_force, stop_price=None,
        ...
        order_class="oto" + stop_loss={"stop_price": ...} submits a single
        One-Triggers-Other order: Alpaca holds the stop_loss leg (status
        "held") until the market leg fills, then activates it server-side.
        No second top-level order is ever submitted, which is what avoids
        Alpaca's wash-trade check (error 40310000) for short entries.
        """
        order_side = OrderSide.BUY if side == "buy" else OrderSide.SELL
        tif = TimeInForce(time_in_force.lower())
        qty = float(qty)
        oclass = OrderClass(order_class) if order_class else None
        sl_req = (
            StopLossRequest(stop_price=float(stop_loss["stop_price"]))
            if stop_loss else None
        )
        if type == "market":
            req = MarketOrderRequest(
                symbol=symbol, qty=qty, side=order_side, time_in_force=tif,
                order_class=oclass, stop_loss=sl_req,
            )
        elif type == "stop":
    ...
    def get_order(self, order_id, nested=False):
        filt = GetOrderByIdRequest(nested=nested) if nested else None
        return self._trading.get_order_by_id(str(order_id), filter=filt)
